...
if ($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    // Prepare and execute SQL query
    $fetch_notification = $connect->prepare("SELECT * FROM notifcations WHERE User_id=?") ;
    $fetch_notification->execute(array($userid));
    $row_notfi = $fetch_notification->fetchAll();
    $count_notfi = $fetch_notification->rowCount();
    if($count_notfi > 0){
        // Return JSON response with notifications
        echo json_encode(['status' => 'success', 'message' => 'Data retrieved successfully', 'notifications' => $row_notfi]);
    }else{
        // Return JSON response if no notifications found
        echo json_encode(['status' => 'error', 'message' => 'No notifications found']);
    }
