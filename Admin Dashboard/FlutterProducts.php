...
$fetch_allproduct = $connect->prepare("SELECT * FROM products");
$fetch_allproduct->execute();
$row_allproduct = $fetch_allproduct->fetchAll();
$count_allproduct = $fetch_allproduct->rowCount();
if($count_allproduct > 0){
    echo json_encode($row_allproduct);
}else{
    echo json_encode(["error" => "No products found"]);
}
ob_end_flush();